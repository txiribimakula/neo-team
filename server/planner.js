...
import { eligibleTasks, isExecutable, hierarchy, ancestors, participantSources } from '../dist/hierarchy.js';

export const FIELD_LABELS = { assignedTo: 'Responsable', iterationPath: 'Iteración', priority: 'Prioridad', remainingWork: 'Horas pendientes' };
...
export const effectiveItems = workspace => workspace.items.map(item => ({ ...item, ...workspace.drafts[item.id], modified: !!workspace.drafts[item.id] }));
...
export function planReview(workspace, remoteItems) {
  ...
  return Object.entries(workspace.drafts).map(([id, fields]) => {
...
export class Planner {
  ...
  async prepareReview() {
    ...
    const remoteItems = workspace.mode === 'demo' ? workspace.items : await this.azure.getItems(workspace.config, ids);
    ...
    this.review = { token: randomUUID(), version: this.store.data.version, mode: workspace.mode, plans };
  ...
  async sync(token) {
    ...
    if (workspace.mode === 'azure') {
      ...
      const current = await this.azure.getItems(workspace.config, review.plans.map(p => p.id));
      if (review.plans.some(p => current.find(i => i.id === p.id)?.rev !== p.remote.rev)) throw new Error('Azure DevOps ha cambiado desde la revisión. Revisa de nuevo antes de sincronizar.');
    }
