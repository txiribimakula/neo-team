...
export async function auditGroup(call, catalog, descriptor, progress = () => {}) {
  ...
  async function attempt(name, fn) {
    ...
    catch (error) { if (error.code !== 'AZURE_SECURITY_SOURCE_DENIED' && (error.code === 'AZURE_AUTHENTICATION_REQUIRED' || /\bAzure HTTP 401\b/.test(error.message))) throw error; counts.warnings++; report.coverage.push({ name, status: 'error', message: error.message, ...(error.diagnostics ? { diagnostics: error.diagnostics } : {}) }); return null; }
  }
