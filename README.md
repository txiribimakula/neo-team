...
5. En **2 · Revisar anterior**, revisa lo que sigue abierto en la iteración anterior: cada persona ocupa una fila y, a su derecha, sus tareas y bugs en horizontal con su estado y sus horas pendientes. **Pasar a …** mueve la tarea a la iteración elegida conservando su responsable. **Completada** la cierra con el estado completado de su tipo (por ejemplo, `Closed` o `Done`). **Backlog** la devuelve al backlog del equipo, sin iteración y conservando su responsable. Las tres decisiones se guardan en local, se pueden deshacer y se envían a Azure DevOps al sincronizar. La pestaña indica cuántas tareas quedan por decidir.
