...
5. En **2 · Revisar anterior**, revisa lo que sigue abierto en la iteración anterior, agrupado por persona: cada tarjeta muestra las tareas y bugs de esa persona con su estado y sus horas pendientes. **Pasar a …** mueve la tarea a la iteración elegida conservando su responsable. **Marcar completada** la cierra con el estado completado de su tipo (por ejemplo, `Closed` o `Done`). Ambas decisiones se guardan en local, se pueden deshacer y se envían a Azure DevOps al sincronizar. La pestaña indica cuántas tareas quedan por decidir.
6. En **3 · Capacidad**, revisa y ajusta las horas por día y los días libres de cada persona en la iteración seleccionada, además de los días libres comunes del equipo. Cada cambio se guarda en local, recalcula al instante las horas disponibles del paso 5 y se envía a Azure DevOps al sincronizar. Los días libres deben estar dentro de la iteración y no pueden solaparse entre sí.
7. En **4 · Repartir ramas**, abre el desplegable de personas de una Epic, Feature, User Story o tarea. Busca nombres y marca o desmarca sus casillas. El reparto se guarda al instante en local y se hereda a los descendientes.
8. En **5 · Elegir tareas**, navega por las tarjetas de personas. Cada una muestra sus horas, una barra de carga y si queda margen, están cubiertas o se ha superado la capacidad. Si faltan estimaciones o capacidad, el indicador lo señala. Quienes tienen las horas cubiertas o excedidas aparecen al final. Con las tareas estimadas, su botón **Confirmar** marca el reparto como revisado en esta iteración; se guarda en local y se invalida si cambian sus tareas, horas o capacidad.
9. Marca las tareas y bugs que corresponden a la persona en la iteración. **La casilla refleja el plan guardado**: no hay botón de añadir ni selección pendiente de confirmar. Desmarcar deja la tarea sin responsable y la devuelve al backlog, conservando sus demás cambios. Las seleccionadas permanecen visibles para poder quitarlas; las que tienen otro responsable están ocultas por defecto.
10. **Quitar rama** permite retirar un bloque completo desde el paso 5. Retira la participación de esa persona en el bloque y sus descendientes, y desmarca sus tareas de la iteración actual. Conserva los demás participantes y no modifica las asignaciones de otras iteraciones. Se puede volver a incluir una subrama expresamente desde el paso 4. Reincorporar una rama no vuelve a seleccionar sus tareas automáticamente.
11. **6 · Revisar** compara el borrador (tareas y capacidad) con Azure DevOps y permite confirmar la sincronización. Si la capacidad ha cambiado en Azure desde la importación, la revisión lo señala como conflicto y ofrece conservar la versión de Azure o mantener la tuya. Los desplegables y casillas anteriores nunca escriben directamente en Azure.
